...
print_all_consoles :- portable(X, Portable), commercial_failure(X, ComFail), has_3d_support(X, Has3D), difficult_development(X, DifficultDev), motion_controls(X, MotionControls),
	backwards_compatible(X, BackwardsCompatible), optical_media(X, HasDiskReader),
	print(X), write(' '), print(Portable), print(ComFail), print(Has3D), print(DifficultDev), print(MotionControls), print(BackwardsCompatible),
	print(HasDiskReader), nl, fail.
